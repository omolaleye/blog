...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        return render_template("select_movie.html", options=data)
...
def flight():
    if request.method == "POST":
        data = request.form
        send_flight_sub(data["name"], data["email"])
        return render_template("flight.html", flight_msg_sent=True, ent_user=current_user)
    return render_template("flight.html", flight_msg_sent=False, current_user=current_user)

def send_flight_sub(name, email):
    flight_sub = f"Subject:New Flight Club Member\n\nName: {name}\nEmail: {email}\n"
    with smtplib.SMTP("smtp.gmail.com") as connection:
        connection.starttls()
        connection.login(MY_EMAIL, MY_PASSWORD)
        connection.sendmail(MY_EMAIL, MY_EMAIL, flight_sub)
